...
from trlib.modules.char_embedding import CharEmbedding
from trlib.modules.embeddings import Embeddings, TreePositionalEncodings, \
                                     PositionalEncoding
from trlib.modules.highway import Highway
from trlib.encoders.transformer import TransformerEncoder
from trlib.decoders.transformer import TransformerDecoder
from trlib.inputters import constants
from trlib.modules.global_attention import GlobalAttention
from trlib.modules.copy_generator import CopyGenerator, CopyGeneratorCriterion
...
class Embedder(nn.Module):
    def __init__(self, args):
        ...
        assert args.use_src_word or args.use_src_char or args.use_code_type
        ...
        self.sum_over_subtokens = args.sum_over_subtokens
        ...
        if self.use_type:
            if self.enc_input_size == 0:
                self.enc_input_size += args.emsize
            self.type_embeddings = nn.Embedding(args.type_vocab_size,
                                                self.enc_input_size)
            
        self.use_type2 = args.use_code_type2
        if self.use_type2:
            if self.enc_input_size == 0:
                self.enc_input_size += args.emsize
            self.type_embeddings2 = nn.Embedding(args.type_vocab_size2,
                                                self.enc_input_size)
            
        self.use_tree_pos_enc = args.use_tree_pos_enc
        if self.use_tree_pos_enc:
            self.tree_pos_encodings = TreePositionalEncodings(self.enc_input_size,\
                                                              args.max_path_width,\
                                                              args.max_path_depth)

        self.src_pos_emb = args.src_pos_emb
        self.src_pos_enc = args.src_pos_enc and not self.src_pos_emb
        self.tgt_pos_emb = args.tgt_pos_emb

        if self.src_pos_emb:
            self.src_pos_embeddings = nn.Embedding(args.max_src_len,
                                                   self.enc_input_size)
        elif self.src_pos_enc:
            self.src_pos_encodings = PositionalEncoding(dropout=0,\
                                                       dim=self.enc_input_size,\
    ...
    def forward(self,
        ...
        if mode == 'encoder':
            word_rep = None
            if self.use_src_word:
                word_rep = self.src_word_embeddings(sequence.unsqueeze(2))
                if self.sum_over_subtokens:
                    mask_rep = (word_rep != constants.PAD).float()
                    word_rep = word_rep.sum(dim=2) / torch.max(mask_rep.sum(dim=2),\
                                      torch.FloatTensor([1.0]).to(word_rep.device))  
                # B x P x d
            ...
            if self.use_type2:
                type_rep2 = self.type_embeddings2(sequence_type2)
                if word_rep is None:
                    word_rep = type_rep2
                else:
                    word_rep = word_rep + type_rep2
            ...
            if self.src_pos_emb:
                ...
                word_rep = self.src_pos_encodings(word_rep)
                
            if self.use_tree_pos_enc:
                tree_pos_enc = self.tree_pos_encodings(sequence_root_paths) # batch, seq, emb
                word_rep = word_rep + tree_pos_enc
...
class Encoder(nn.Module):
    def __init__(self,
        ...
        ggnn_layers_info = {} if not args.use_ggnn_layers else \
                           {"n_edge_types": args.n_edge_types, \
                            "n_steps_ggnn": args.n_steps_ggnn, \
                            "ggnn_first": args.ggnn_first}
        self.transformer = TransformerEncoder(num_layers=args.nlayers,
                          ...
                          args.use_tree_relative_attn,\
                          tree_rel_vocab_size=\
                          args.tree_rel_vocab_size,\
                          ggnn_layers_info=ggnn_layers_info,\
                          type_vocab_size=args.type_vocab_size,\
                          type_vocab_size2=args.type_vocab_size2)
    ...
    def forward(self,
                ...
                input_tokens=None,
                adj_matrices=None):
        layer_outputs, _ = self.transformer(input, input_len, \
                                            rel_matrix, input_type, input_type2, input_tokens, adj_matrices)  
                           # B x seq_len x h
        if self.use_all_enc_layers:
...
class Transformer(nn.Module):
    ...
    def __init__(self, args, tgt_dict):
        ...
        if args.share_decoder_encoder_embeddings:
            if self.embedder.use_tgt_word and self.embedder.use_src_word:
                assert self.embedder.enc_input_size == self.embedder.dec_input_size
                self.embedder.tgt_word_embeddings.word_lut.weight = \
                    self.embedder.src_word_embeddings.word_lut.weight
    ...
    def _run_forward_ml(self,
        ...
        code_rep = self.embedder(code_word_rep,
                                 ...
                                 code_root_paths_rep,
                                 mode='encoder')
        memory_bank, layer_wise_outputs = self.encoder(code_rep, code_len, code_rel_matrix_rep, code_type_rep, code_type2_rep, code_word_rep, adj_matrices)  # B x seq_len x h
    ...
    def decode(self,
        ...
        word_rep = self.embedder(code_word_rep,
                                 ...
                                 code_root_paths_rep,
                                 mode='encoder')
        memory_bank, layer_wise_outputs = self.encoder(word_rep, code_len, code_rel_matrix_rep, code_type_rep, code_type2_rep, code_word_rep, adj_matrices)  # B x seq_len x h
